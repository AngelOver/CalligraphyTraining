...
MgrUser.openAddMgr = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: Feng.ctxPath + '/scheduleAdmin/user_add'
    });
...
MgrUser.openChangeUser = function () {
    ...
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/scheduleAdmin/user_edit/' + this.seItem.id
        });
        this.layerIndex = index;
    }
};
...
MgrUser.roleAssign = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/scheduleAdmin/role_assign/' + this.seItem.id
        });
        this.layerIndex = index;
    }
};
...
MgrUser.delMgrUser = function () {
    if (this.check()) {
        ...
        var operation = function(){
            ...
            var ajax = new $ax(Feng.ctxPath + "/scheduleAdmin/delete", function () {
                Feng.success("删除成功!");
                MgrUser.table.refresh();
            }, function (data) {
...
MgrUser.freezeAccount = function () {
    if (this.check()) {
        var userId = this.seItem.id;
        var ajax = new $ax(Feng.ctxPath + "/scheduleAdmin/freeze", function (data) {
            Feng.success("冻结成功!");
            MgrUser.table.refresh();
        }, function (data) {
...
MgrUser.unfreeze = function () {
    if (this.check()) {
        var userId = this.seItem.id;
        var ajax = new $ax(Feng.ctxPath + "/scheduleAdmin/unfreeze", function (data) {
            Feng.success("解除冻结成功!");
            MgrUser.table.refresh();
        }, function (data) {
            Feng.error("解除冻结失败!");
...
MgrUser.resetPwd = function () {
    if (this.check()) {
        ...
        parent.layer.confirm('是否重置密码为111111？', {
            ...
            var ajax = new $ax(Feng.ctxPath + "/scheduleAdmin/reset", function (data) {
                Feng.success("重置密码成功!");
            }, function (data) {
                Feng.error("重置密码失败!");
            });
...
$(function () {
    ...
    var table = new BSTable("managerTable", "/scheduleAdmin/list?deptid=30", defaultColunms);
